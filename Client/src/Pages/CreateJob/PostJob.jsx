...
function PostJob() {
  ...
  const [department, setDepartments] = useState(["IT", "HR"]);
  ...
  console.log(count);
  var org_data = [];
  org_data.push(count?.[8][1] || "Casablanca");
  org_data.push(count?.[9][1] || "Maroc");
  org_data.push(count?.[2][1] || "ENSA",);
  org_data.push(count?.[0][1] || "6588d43430a59ac4eeb094bf");
  ...
  const handleSubmit = async () => {
    // axios POST request
    const options = {
      url: import.meta.env.VITE_API_URL + "/job/post",
      method: "POST",
      headers: {
