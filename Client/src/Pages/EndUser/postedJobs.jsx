...
function PostedJobs() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        import.meta.env.VITE_API_URL + "/job/get-all-jobs"
      );
  ...
  // console.log(data);
  return (
    <div className="p-0 w-full sm:p-6 bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900">
      <div className="p-0 sm:p-20  flex justify-between">
        <div className="p-20 sm:0 ">
          ...
          <h2 className="text-blue-500 heading1 inline">Dream</h2>
          ...
          text-blue-500 heading1 inline ml-5 bg-transparent border border-solid border-blue-500 shadow-sm shadow-gray-200 rounded-md p-2 relative z-0"
          >
            Get Hired
          </h2>
        </div>
      ...
      <footer className="bg-gray-100 bg-opacity-10 text-center heading4 text-gray-200 mt-16 rounded-lg p-2">
        Powerd By : <p className="inline  font-medium"> KHEDMTY</p>
      </footer>
    </div>
  );
}
